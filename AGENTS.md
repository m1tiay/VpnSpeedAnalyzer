...
### Release Versioning Contract (обязательно при подготовке релиза)

- При подготовке нового релиза агент обязан обновить версию приложения (`Version`, `AssemblyVersion`, `FileVersion`, `InformationalVersion`) под целевой релиз.
- Перед публикацией релиза агент обязан проверить, что версия в окне `О приложении` совпадает с версией релизного тега GitHub.
- При подготовке релиза агент обязан обновить содержимое окна `О приложении` (блок `Что нового`) в соответствии с фактическими изменениями, вошедшими в релиз.
- Перед завершением релизной задачи агент обязан явно отчитаться:
  - какая версия установлена в приложении,
  - какой тег/релиз публикуется,
  - что версия в приложении и релизе совпадает.
